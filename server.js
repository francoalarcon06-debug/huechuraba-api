...
app.post("/auth/login", async (req, res) => {
  try {
    const { correo, contrasenia } = req.body; // "contrasenia" para evitar la ñ en el JSON
    if (!correo || !contrasenia) {
      return res.status(400).json({ ok: false, error: "Faltan datos" });
    }

    const q = `
      SELECT id, correo, contrasenia AS password, nombre, rut, cargo, departamento, telefono, activo
      FROM personal_municipalidad
      WHERE correo = $1
      LIMIT 1
    `;
    ...
    // (Opcional) firma JWT, por ahora te devuelvo datos simples
    return res.json({
      ok: true,
      user: {
        id: user.id,
